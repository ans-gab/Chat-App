export const requestModel = [{
  ...
  selectable: false,
  children: [
    { title: 'gpt-3.5-turbo', value: 'gpt-3.5-turbo', description: '功能最强大的 GPT-3.5 模型，并针对聊天进行了优化，成本仅为text-davinci-003的十分之一. 将使用我们最新的模型迭代进行更新。', max_tokens: 4000, preurl: '/v1/chat/completions' },
    { title: 'text-davinci-003', value: 'text-davinci-003', description: '功能最强大的 GPT-3 模型。可以完成其他模型可以完成的任何任务，通常具有更高的质量、更长的输出和更好的指令遵循。还支持在文本中插入补全。', max_tokens: 4000, preurl: '/v1/completions' },
    { title: 'text-curie-001', value: 'text-curie-001', description: '非常有能力，但比Davinci更快，成本更低。', max_tokens: 2048, preurl: '/v1/completions' },
    { title: 'text-babbage-001', value: 'text-babbage-001', description: '能够执行简单的任务，速度非常快，成本更低。', max_tokens: 2048, preurl: '/v1/completions' },
    { title: 'text-ada-001', value: 'text-ada-001', description: '能够执行非常简单的任务，通常是 GPT-3 系列中最快的型号，而且成本最低。', max_tokens: 2048, preurl: '/v1/completions' },
  ]
},
{
  title: 'Davinci',
...
},
{
  ...
  selectable: false,
  children: [
    { title: 'code-cushman-001', value: 'code-cushman-001', description: '几乎与 Davinci Codex 一样强大，但速度稍快。这种速度优势可能使其成为实时应用程序的首选。', max_tokens: 2048, preurl: '/v1/completions' },
    { title: 'code-davinci-002', value: 'code-davinci-002', description: '功能最强大的 Codex 型号。特别擅长将自然语言翻译成代码。除了补全代码，还支持在代码中插入补全。', max_tokens: 8000, preurl: '/v1/completions' },
...
export const requestParamsInfo = [
  {
    title: 'MaxTokens',
    description: '指定生成的响应的最大标记数。标记是指模型生成的单词、标点符号和其他符号。默认值为512，建议在10到2048之间进行调整。'
  },
  {
    title: 'temperature',
    description: '"temperature"选项是一个控制语言模型输出的随机性或创造性的参数。当使用语言模型生成文本时，它通常会输出根据输入和先前训练数据确定为最可能的单词或词序列。然而，增加输出的随机性可以帮助模型创建更具创意和有趣的输出。"temperature"选项实际上控制着随机性的程度。将温度设置为较低的值将导致输出更可预测和重复，而较高的温度会导致更多种类和不可预测的输出。例如，将温度设置为0.5将导致较保守的输出，而温度为1将创建更富创意和自发的输出。需要注意的是，理想的温度值将取决于具体的任务和上下文，因此可能需要一些实验来找到适合您需要的正确值。'
  },
  {
    title: 'top_p',
    description: '0.0 to 1.0 (默认 1.0) 使用温度的另一种选择，也叫核采样（nucleus sampling），建议不要同时使用temperature和top_p。"top_p"与另一个称为“top_k”的参数有关，它将模型限制为仅考虑最可能的前k个标记。这些选项一起提供了更多对生成文本多样性和质量的控制。'
  },
  {
    title: 'frequency_penalty',
    description: '控制生成的响应中是否包含与初始提示重复的主题。较高的频率惩罚值会导致生成的响应更加独特，但可能会降低响应的相关性。默认值为0，建议在0到1之间进行调整。'
  },
  {
    title: 'presence_penalty',
    description: '控制生成的响应中是否包含与初始提示不相关的主题。较高的存在惩罚值会导致生成的响应更加相关，但可能会降低响应的创造性。默认值为0，建议在0到1之间进行调整。'
  }
]